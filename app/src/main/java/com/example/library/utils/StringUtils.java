...
public class StringUtils {
     ...
     * 验证两个值是否相同
     *
     * @param value1 first CharSequence to check
     * @param value2 second CharSequence to check
     * @return true if value1 and value2 are equal
     */
    public static boolean checkValSame(CharSequence value1, CharSequence value2) {
    ...
    // 秒转小时:分:秒
    public static String long2Str(long sec) {
        if (sec < 60) {// 小于1分钟
            return sec + "秒";
        } else if (sec < 60 * 60) {// 小于1小时
            long min = sec / 60;    //分钟
            sec = sec % 60;        //秒
            return min + "分钟" + sec + "秒";
        } else {// 小于1天
            long hour = sec / 3600;  // 小时
            long min  = sec % 3600 / 60;    //分钟
            sec = sec % 3600 % 60;        //秒
